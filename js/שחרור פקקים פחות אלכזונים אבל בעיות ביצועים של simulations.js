...
// 3. תנועה חכמה עם "עקיפת פקקי תנועה" (Traffic Dispersion)
                    if (minDist > 0 && minDist !== Infinity) {
                        ...
                        // אוספים את *כל* השכנים שמקרבים אותנו למטרה (לא רק את האחד המושלם)
                        for (const {dr, dc} of neighbors) {
                            const nr = row + dr;
                            const nc = col + dc;
                            if (nr >= 0 && nr < n && nc >= 0 && nc < n) {
                                ...
                                // האם השכן הזה באמת מקרב אותנו לעומת איפה שאנחנו עכשיו?
                                if (neighborDistSq < currentDistSq) {
                                    potentialMoves.push({ nr, nc, distSq: neighborDistSq });
                                }
                            }
                        }

                        // ממיינים את רשימת המסלולים: מהמסלול המהיר ביותר להכי פחות מהיר
                        potentialMoves.sort((a, b) => a.distSq - b.distSq);

                        // 4. תנועה: בהסתברות של 80%, ננסה למצוא נתיב פנוי מתוך 3 המסלולים הטובים ביותר
                        if (potentialMoves.length > 0 && Math.random() < 0.8) {
                            const maxAttempts = Math.min(3, potentialMoves.length);
                            
                            for (let attempt = 0; attempt < maxAttempts; attempt++) {
                                const move = potentialMoves[attempt];
                                const target_i = move.nr * n + move.nc;
                                
                                // בדיקת "פקק": האם המשבצת באמת פנויה (בהירה יותר, לא זהב, ועוד לא זזה)?
                                if (!nextBoardState[target_i].isGold &&
                                    nextBoardState[i].k < nextBoardState[target_i].k && 
                                    !movedThisFrame.has(target_i)) {
                                    
                                    // מצאנו נתיב פנוי! מבצעים החלפה
                                    [nextBoardState[i], nextBoardState[target_i]] = [nextBoardState[target_i], nextBoardState[i]];
                                    
                                    movedThisFrame.add(i);
                                    movedThisFrame.add(target_i);
                                    
                                    // ברגע שביצענו תנועה מוצלחת, יוצאים מהלולאה (לא בודקים עוד מסלולים)
                                    break;
                                }
                            }
                        }
